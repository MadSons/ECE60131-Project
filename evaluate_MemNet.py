...
class Validator():
    def __init__(self, config):
        ...
        loaded_obj = torch.load(config.model, map_location='cpu', weights_only=False)

        if isinstance(loaded_obj, torch.nn.Module):
            # The file was a pickled model
            self.mem_n2n = loaded_obj
        elif isinstance(loaded_obj, dict) and all(isinstance(v, torch.Tensor) for v in loaded_obj.values()):
            ...
            # load AE weights
            self.ae = model_encdec(self.settings)
            self.ae.load_state_dict(torch.load(config.model_ae, map_location='cpu'), strict=False)

            # build controller and load its weights
            self.controller = model_controllerMem(self.settings, self.ae)
            self.controller.load_state_dict(
                torch.load(config.model_controller, map_location='cpu'),
                strict=False
            )

            # build MANTRA and load the MANTRA state_dict
            self.mem_n2n = model_memory_IRM(self.settings, self.controller)
            self.mem_n2n.load_state_dict(loaded_obj, strict=False)
        else:
            raise RuntimeError(
                f"Unrecognized object in torch.load({config.model!r}): "
    ...
    def _memory_writing(self, saved_memory):
        ...
        if saved_memory:
            self.mem_n2n.memory_past = torch.load(self.config.memories_path + 'memory_past.pt', map_location=self.device)
            self.mem_n2n.memory_fut = torch.load(self.config.memories_path + 'memory_fut.pt', map_location=self.device)
        else:
