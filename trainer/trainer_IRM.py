...
class Trainer:
    def __init__(self, config):
        ...
        self.ae.load_state_dict(torch.load(config.model_ae, map_location=self.device))

        self.controller = model_controllerMem(self.settings, self.ae)
        self.controller.load_state_dict(torch.load(config.model_controller, map_location=self.device))

        #self.model = torch.load(config.model)
        self.mem_n2n = model_memory_IRM(self.settings, self.controller).to(self.device)
        self.mem_n2n.past_len = config.past_len
        self.mem_n2n.future_len = config.future_len

        self.criterionLoss = nn.MSELoss().to(self.device)
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            ...
            if (epoch + 1) % 5 == 0:
                ...
                val_loss = dict_metrics_test['euclMean']
                if val_loss < self.best_val_loss:
                    self.best_val_loss = val_loss
                    torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_IRM_best_' + self.name_test)

                # Save model checkpoint
                if (epoch + 1) % 20 == 0:
                #    torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_IRM_epoch_' + str(epoch+1) + '_' + self.name_test + '.pt')
        ...
        torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_IRM_' + self.name_test + '.pt')
