...
class Trainer():
    def __init__(self, config):
        ...
        self.model_ae = model_encdec(self.settings).to(self.device)
        self.model_ae.load_state_dict(torch.load(config.model_ae))
        self.mem_n2n = model_controllerMem(self.settings, self.model_ae).to(self.device)
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            ...
            if (epoch + 1) % 5 == 0:
                ...
                if val_loss < self.best_val_loss:
                    ...
                    torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_controller_best_' + self.name_test)

                # Save model checkpoint
                if (epoch + 1) % 20 == 0:
                    #torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_controller_epoch_' + str(epoch) + '_' + self.name_test)
        ...
        torch.save(self.mem_n2n.state_dict(), self.folder_test + 'model_controller_' + self.name_test)
